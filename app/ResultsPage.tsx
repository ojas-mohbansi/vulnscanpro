...
import { AlertCircle, Search, Filter, ChevronRight, ArrowLeft, Loader2, ChevronDown, GraduationCap, FileCheck, TrendingUp, Lightbulb, Clock, Activity, Shield, Bell, WifiOff } from 'lucide-react';
...
const ResultsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for local DB updates from SyncService
    const handleDbUpdate = (e: any) => {
      if (e.detail?.id === id) {
        api.getScan(id).then(setScan);
      }
    };
    window.addEventListener('scan-updated', handleDbUpdate);
    ...
    // Subscribe to SSE if online
    // If offline, this returns a no-op cleanup
    const unsubscribe = api.subscribeToScan(id, (event) => {
      ...
      api.getScan(id).then(async (s) => {
        ...
        // Notification Logic for High Severity Findings
    ...
    return () => {
        unsubscribe();
        window.removeEventListener('scan-updated', handleDbUpdate);
    };
  }, [id, isOnline]); // Re-run when network status changes to attempt reconnect

  const handleRequestPermission = async () => {
    const granted = await NotificationService.requestPermission();
    if (granted) {
      setNotifPermission('granted');
      // Update preferences to ensure enabled
      const prefs = PreferencesService.getPreferences();
      prefs.notifications.browserNotifications = true;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-4 md:p-6 lg:p-8">
      <div className="max-w-[1920px] mx-auto space-y-6">
        ...
        {/* Offline Warning for Active Scan */}
        {!isOnline && scan.status === 'running' && (
           <div className="bg-amber-500/10 border border-amber-500/20 text-amber-500 p-4 rounded-xl flex items-center gap-3 animate-in slide-in-from-top">
              <WifiOff className="w-5 h-5" />
              <div>
                 <p className="font-bold">Scan Paused (Offline)</p>
                 <p className="text-xs opacity-90">Progress stream disconnected. Reconnect to internet to resume monitoring. Findings are cached.</p>
              </div>
           </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
           ...
           <div className="w-full sm:w-auto flex flex-wrap items-center justify-end gap-3">
             {scan.status === 'running' && notifPermission === 'default' && (
               <button 
                 onClick={handleRequestPermission}
                 className="flex items-center gap-2 px-3 py-2 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 hover:text-indigo-300 rounded-lg border border-indigo-500/20 transition-all text-sm font-medium animate-in fade-in"
               >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          {/* Scales: Full width on mobile, 4/12 cols on laptop, 3/12 cols on large screens */}
          <div className="space-y-6 lg:col-span-4 xl:col-span-3 lg:sticky lg:top-6 lg:h-fit transition-all duration-300">
          ...
          {/* Scales: Full width on mobile, 8/12 cols on laptop, 9/12 cols on large screens */}
          <div className="lg:col-span-8 xl:col-span-9 bg-slate-900 border border-slate-800 rounded-xl shadow-lg flex flex-col overflow-hidden min-h-[600px] lg:h-[calc(100vh-100px)]">
             {/* Tabs */}
                         ...
                         <div className="hidden md:block w-full">
                           <table className="w-full text-left border-collapse">
                             <thead className="bg-slate-900/90 sticky top-0 z-10 text-xs font-semibold text-slate-400 uppercase tracking-wider shadow-sm">
                               <tr>
                                 <th className="px-4 lg:px-6 py-4 border-b border-slate-800">Severity</th>
                                 <th className="px-4 lg:px-6 py-4 border-b border-slate-800 w-1/3">Finding</th>
                                 <th className="px-4 lg:px-6 py-4 border-b border-slate-800">Module</th>
                                 <th className="px-4 lg:px-6 py-4 border-b border-slate-800">Risk Score</th>
                                 <th className="px-4 lg:px-6 py-4 border-b border-slate-800">Confidence</th>
                                 <th className="px-4 lg:px-6 py-4 border-b border-slate-800 text-right">Time</th>
                               </tr>
                             </thead>
                             <tbody className="divide-y divide-slate-800 text-sm">
                               {visibleFindings.map((finding) => (
                                 ...
                                 >
                                   ...
                                   <td className="px-4 lg:px-6 py-4">
                                     <div className="font-medium text-slate-200 group-hover:text-white truncate max-w-[200px] lg:max-w-[300px]">{finding.title}</div>
                                     <div className="text-xs text-slate-500 mt-1 truncate max-w-[200px] lg:max-w-[300px]">{finding.description}</div>
                                   </td>
                                   <td className="px-4 lg:px-6 py-4">
                                   ...
                                   <td className="px-4 lg:px-6 py-4 text-right text-xs font-mono text-slate-600">
