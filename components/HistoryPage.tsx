              ...
              <tbody className="divide-y divide-slate-800">
                {history.map((scan) => (
                  <tr key={scan.id} className={`hover:bg-slate-800/50 transition-colors ${selectedForDiff.includes(scan.id) ? 'bg-slate-800/30' : ''}`}>
                    ...
                    <td className="px-6 py-4 font-mono text-slate-300 truncate max-w-[200px]" title={scan.target}>
                      {scan.target}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${
                         scan.status === 'completed' ? 'bg-green-900/30 text-green-400' : 'bg-slate-800 text-slate-400'
                      }`}>
                        {scan.status}
                      </span>
                    </td>
